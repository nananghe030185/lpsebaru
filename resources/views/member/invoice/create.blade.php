...
@section('main_content')
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-12">
                 <form class="row g-3" action="{{ route('app.invoice.store') }}" method="POST" enctype="multipart/form-data">
                    <div class="card">
                        ...
                        <div class="card-body">
                                @csrf
                                {{-- @method('PUT') --}}
                                @include('member.invoice.form')
                        </div>
                        <div class="card-footer text-end">
                            <button type="submit" name="submit" class="btn btn-primary">{{ __('Simpan') }}</button>
                            <a href="{{ route('app.invoice.index')}}" class="btn btn-danger">{{ __('Batal')}}</a>
                        </div>
                    </div>
                </form>
            </div>
