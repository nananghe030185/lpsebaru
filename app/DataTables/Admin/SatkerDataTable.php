...
class SatkerDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->editColumn('lelang', function ($row) {
                return view('admin.inc.action', [
                    'toggle' => $row,
                    'name' => 'lelang',
                    'route' => 'admin.satuan-kerja.lelang',
                    'value' => $row->lelang,
                ]);
            })
            ->editColumn('swakelola', function ($row) {
                return view('admin.inc.action', [
                    'toggle' => $row,
                    'name' => 'swakelola',
                    'route' => 'admin.satuan-kerja.swakelola',
                    'value' => $row->swakelola,
                ]);
            })
            ->editColumn('action', function ($row) {
                return view('admin.inc.action', [
                    'edit'   => 'admin.satuan-kerja.edit',
                    'data'   => $row
                ]);
            })
    ...
    public function html(): HtmlBuilder
    {
        return $this->builder()
                    ...
                    ->addAction(['className' => 'text-center'])
                    ->buttons([
                        Button::make('excel')
